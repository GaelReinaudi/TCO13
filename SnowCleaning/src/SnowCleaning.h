...
vector<string> SnowCleaning::nextDay(vector<int> snowFalls) {
	...
	maxRC = pBoard->MaxSnowExt();
	double valTrig = 70.0;
	double distTrig = sqrt(radius) * 240.0 / pBoard->ValExt(maxRC[0], maxRC[1]);
	...
	maxRC = pBoard->MaxSnowExt();
	valTrig = 80.0;
	distTrig = sqrt(radius) * 100.0 / pBoard->ValExt(maxRC[0], maxRC[1]);
	if((pBoard->ValExt(maxRC[0], maxRC[1]) >= valTrig))
	{
		if(pBoard->ClosestWorker(maxRC[0], maxRC[1]) > distTrig && DiceHire1()) {
			string order;
			order = Hire(maxRC[0], maxRC[1]);
			if(order != "")
				orders.push_back(order);
...
string Worker::MoveForce() {
	...
	FOR(otherR, r-range, r+range) {
		FOR(otherC, c-range, c+range) {
			...
			if(dist == 0.0 || otherC < 0 || otherC >= l || otherR < 0 || otherR >= l)
				continue;
	...
	for(std::map<int, Worker*>::iterator it = pBoard->idWorkers.begin(); it != pBoard->idWorkers.end(); ++it) {
		...
		if(dist == 0.0)
			continue;
		double fac = 1.0;// + (double(rand()) / double(RAND_MAX) - 0.5) * 0.4;
